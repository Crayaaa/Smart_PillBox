...
#include "uart_bridge.h"                     // UART桥接接口
...
// 存储从客户端接收到的数据
#define RECEIVED_DATA_MAX_LEN 256
static char g_received_data[RECEIVED_DATA_MAX_LEN] = {0};
static uint8_t g_received_data_len = 0;
                             ...
                             errcode_t status)
{
    ...
    // 每次接收到SLE数据都更新存储数组（不比较差异）
    // 检查数据长度是否超出最大限制
    if (write_cb_para->length < RECEIVED_DATA_MAX_LEN) {
        // 计算有效负载长度（不包含可能的字符串结束符'\0'）
        uint8_t payload_len = write_cb_para->length;
        if (payload_len > 0 && write_cb_para->value[payload_len - 1] == '\0') {
            payload_len--;
        }
        // 清空旧数据
        ...
        // 复制新数据
        if (memcpy_s(g_received_data, sizeof(g_received_data), 
                    write_cb_para->value, payload_len) == EOK) {
            g_received_data_len = payload_len;
            g_received_data[g_received_data_len] = '\0';  // 便于日志打印
            
            printf("%s [ssaps_write_request_cbk] Data stored: %s (len:%d)\r\n", 
                   SLE_SERVER_LOG, g_received_data, g_received_data_len);
            // 不在回调中通过UART发送，改由UART任务发送一次后清空
        } else {
            printf("%s [ssaps_write_request_cbk] Failed to copy new data!\r\n", SLE_SERVER_LOG);
        }
    } else {
        printf("%s [ssaps_write_request_cbk] Data too long! Max length: %d, received: %d\r\n", 
               SLE_SERVER_LOG, RECEIVED_DATA_MAX_LEN, write_cb_para->length);
    }
 ...
 * @return 实际数据长度，0表示没有数据
 */
uint8_t sle_get_received_data(char *buffer, uint8_t buffer_size)
{
    if (buffer == NULL || buffer_size == 0) {
        return 0;
    }
    
    if (g_received_data_len == 0) {
        return 0;
    }
    
    // 确保不会溢出
    uint8_t copy_len = (g_received_data_len < buffer_size) ? g_received_data_len : buffer_size - 1;
    
    if (memcpy_s(buffer, buffer_size, g_received_data, copy_len) == EOK) {
        buffer[copy_len] = '\0';  // 确保字符串结束符
        return copy_len;
    }
 ...
 * @return 数据长度
 */
uint8_t sle_get_received_data_length(void)
{
 ...
 * @brief 清空接收到的数据
 */
void sle_clear_received_data(void)
{
    memset(g_received_data, 0, sizeof(g_received_data));
    g_received_data_len = 0;
    printf("%s [sle_clear_received_data] Received data cleared\r\n", SLE_SERVER_LOG);
}
